...
module.exports = (bluebird, error, logger, requestOptions, requestPromise, responseHandler, URI) => {
    ...
    function validateSearchItem(query) {
        ...
        if (typeof query === "string") {
            ...
            if (!searchValidationRegex.test(query)) {
                return bluebird.reject(new Error("Search query must contain at least one \"=\" not preceeded by a \"\\\".", error.FAILED_QUERY_VALIDATION));
            }

            return bluebird.resolve();
        }

        return bluebird.reject(new Error("Search query must be a string.", error.FAILED_QUERY_VALIDATION));
    }
    ...
    class ArtifactSearch {
         ...
         *                                      for the search criteria. Can be
         *                                      undefined.
         * @property {(string|string[])} sort A string or array of strings for
