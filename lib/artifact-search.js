    ...
    function validateSearchItem(query) {
        ...
        if (typeof query === "string") {
            ...
            if (!searchValidationRegex.test(query)) {
                return bluebird.reject(new ShelfError(`Search query must contain at least one \"=\" not preceeded by a \"\\\". Query Provided: ${query}`, error.FAILED_QUERY_VALIDATION
                ));
            }
            ...
            return validateSearch(searchParameters.search).then(() => {
                ...
                return requestPromise.post(options).then((response) => {
                    ...
                    urls.forEach((val, index) => {
                        urls[index] = responseHandler.resolveLink(options.url, val);
                    });
