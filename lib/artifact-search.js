...
module.exports = (bluebird, error, ExtendedError, logger, requestOptions, requestPromise, responseHandler, URI) => {
    ...
    function validateSearchItem(query) {
        ...
        if (typeof query === "string") {
            ...
            if (!searchValidationRegex.test(query)) {
                return bluebird.reject(new ExtendedError({
                    message: "Search query must contain at least one \"=\" not preceeded by a \"\\\".",
                    code: error.FAILED_QUERY_VALIDATION
                }));
            }
        ...
        return bluebird.reject(new ExtendedError("Search query must be a string.", error.FAILED_QUERY_VALIDATION));
    }
