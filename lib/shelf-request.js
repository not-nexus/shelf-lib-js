    ...
    class ShelfRequest {
        ...
        attachGenericHandlers(req) {
            return req.then((response) => {
                ...
                if (reason.response) {
                    this.logResponse(reason.response, req.url);
                }
        ...
        /**
         * Gets the content of the body as a string if it exists.
         *
        ...
        stringifyBody(body) {
            ...
            if (body instanceof Buffer) {
                // We don't want to log 5GB of response body.
                entry = `(Buffer of length ${body.length})`;
            } else if (typeof body === "string") {
                // In case we get something like a very large HTML file.
                entry = body.substring(0, 1000);
            } else if (body && Object.keys(body).length) {
                /* I check the length of the body's keys because if the response
                 * is empty (and Content-Type is either json or missing) superagent
                 * will default it to an empty object.
                 */
                entry = JSON.stringify(body);
            }
        ...
        logRequest(req) {
            ...
            reqString += this.stringifyBody(req._data);
        ...
        logResponse(res, uri) {
            ...
            resString += this.stringifyBody(res.body);
