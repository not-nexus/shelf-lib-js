...
module.exports = (bluebird, error, logger, parseLinkHeader, ShelfError, URI) => {
    /**
    ...
    function createErrorForResponse(errorResponse) {
        ...
        if (errorCode.length > 0) {
            return new ShelfError(errorResponse.body.message, errorCode);
        }
    ...
    function resolveLinkHeaders(response) {
        ...
        if (response.headers.link) {
            ...
            // If we received just one link header item, push it into an array.
            if (!Array.isArray(links.item)) {
                links = [links.item];
            }

            logger.debug(`Parsed out links: ${JSON.stringify(links, null, 4)}`);
        }

        links.item.forEach((item) => {
            var resolvedLink;

            resolvedLink = resolveLink(response.request.href, item.url);
