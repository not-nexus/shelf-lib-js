...
module.exports = (bluebird, error, HttpLinkHeader, logger, ShelfError, URI) => {
    ...
    function createErrorForResponse(errorResponse) {
        ...
        /* This could be in multiple formats at this point.
         * We only want to deal with the errors if a body
         * exists because that is how we have identified that
         * the error has come from the response instead of an
         * internal error like a socket timeout.
         */
        if (errorResponse.body) {
            errorMessage = errorResponse.body.message || error.UNKNOWN;
        }

        if (errorCode) {
            return new ShelfError(errorMessage, error[errorCode]);
        }

        return new ShelfError(errorMessage, error.UNKNOWN);
    }
