                    ...
                    .on("response", (response) => {
                        ...
                        return response
                            ...
                            .on("finish", () => {
                                ...
                                // Check if we've failed elsewhere, and don't
                                // resolve if we have.
                                if (!failed) {
                                    return resolve();
                                }

                                // We will for sure already have the errorObj
                                // by this point, so we don't need to check.
                                // This is because on("finish") will only be
                                // called if the piping completed successfully,
                                // or after an on("error") is emitted.
                                return reject(errorObj);
                            });
                    })
                    .on("error", (err) => {
